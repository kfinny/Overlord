...
function(findPythonExecutablePath)
  ...
  set(OVERLORD_PYTHON_EXECUTABLE "${Python3_EXECUTABLE}" CACHE INTERNAL "" FORCE)
endfunction()
...
function(generateUnsupportedPlatformSourceFile)
  set(source_file "${CMAKE_CURRENT_BINARY_DIR}/overlord_unsupported_platform_target_source_file.cpp")
...
function(add_overlord_executable)
  set(overlord_exe_options EXCLUDE_FROM_ALL;WIN32;MACOSX_BUNDLE)
  set(overlord_exe_ARGN ${ARGN})

  list(GET overlord_exe_ARGN 0 overlord_exe_name)
  list(REMOVE_AT overlord_exe_ARGN 0)

  foreach(arg ${overlord_exe_ARGN})
    list(FIND overlord_exe_options "${arg}" arg_POS)
    if(${arg_POS} EQUAL -1 AND NOT IS_ABSOLUTE "${arg}")
      set(base_path "${CMAKE_CURRENT_SOURCE_DIR}")
      list(APPEND overlord_exe_args "${base_path}/${arg}")
    else()
      list(APPEND overlord_exe_args "${arg}")
    endif()
  endforeach()

  add_executable(${overlord_exe_name} ${overlord_exe_args})

  if(DEFINED PLATFORM_MACOS)
    getCleanedOverlordVersion("OVERLORD_PLIST_VERSION")
  ...
  if(DEFINED PLATFORM_WINDOWS)
    set(OVERLORD_MANIFEST_TARGET_NAME "${overlord_exe_name}")

    getCleanedOverlordVersion("OVERLORD_MANIFEST_VERSION")

    configure_file(
      "${CMAKE_SOURCE_DIR}/tools/overlord.manifest.in"
      "${overlord_exe_name}.manifest"
      @ONLY NEWLINE_STYLE WIN32
    )
    target_sources(${overlord_exe_name} PRIVATE "${overlord_exe_name}.manifest")
  endif()

  if("${overlord_exe_name}" MATCHES "-test$")
    if(DEFINED PLATFORM_POSIX)
      target_link_options("${overlord_exe_name}" PRIVATE -Wno-sign-compare)
    endif()

    add_dependencies("${overlord_exe_name}" overlord_tools_tests_configfiles)
  endif()
endfunction()

function(add_overlord_library)
  set(overlord_lib_options STATIC;SHARED;MODULE;OBJECT;UNKNOWN;EXCLUDE_FROM_ALL;IMPORTED;GLOBAL;INTERFACE)
  set(overlord_lib_ARGN ${ARGN})

  list(GET overlord_lib_ARGN 0 overlord_lib_name)
  list(REMOVE_AT overlord_lib_ARGN 0)

  foreach(arg ${overlord_lib_ARGN})
    list(FIND overlord_lib_options "${arg}" arg_POS)
    if(${arg_POS} EQUAL -1 AND NOT IS_ABSOLUTE "${arg}")
      set(base_path "${CMAKE_CURRENT_SOURCE_DIR}")
      list(APPEND overlord_lib_args "${base_path}/${arg}")
    else()
      list(APPEND overlord_lib_args "${arg}")
    endif()
  endforeach()

  add_library(${overlord_lib_name} ${overlord_lib_args})
endfunction()
...
function(generateSpecialTargets)
  ...
  add_custom_target(format_check
    COMMAND "${OVERLORD_PYTHON_EXECUTABLE}"
            "${CMAKE_SOURCE_DIR}/tools/formatting/format-check.py"
            --exclude-folders "${excluded_folders}" --binary "${OVERLORD_CLANG_FORMAT}" origin/master
  ...
  add_custom_target(format
    COMMAND "${OVERLORD_PYTHON_EXECUTABLE}"
            ...
            --exclude-folders "${excluded_folders}" --binary "${OVERLORD_CLANG_FORMAT}" -f --style=file
    WORKING_DIRECTORY "${CMAKE_SOURCE_DIR}"
    VERBATIM
  )
endfunction()
...
function(collectInterfaceOptionsFromTarget)
  ...
  cmake_parse_arguments(PARSE_ARGV 0 overlord "" "${oneValueArgs}" "")

  if(NOT overlord_TARGET OR NOT TARGET ${overlord_TARGET})
    message(FATAL_ERROR "A valid target has to be provided")
  endif()

  set(target_list ${overlord_TARGET})
  ...
  while(${target_list_length} GREATER 0)
    foreach(target ${target_list})
      ...
      if(overlord_COMPILE AND NOT "${compile_options}" STREQUAL "compile_options-NOTFOUND")
        list(APPEND compile_options_list ${compile_options})
      endif()

      if(overlord_DEFINES AND NOT "${compile_definitions}" STREQUAL "compile_definitions-NOTFOUND")
        list(APPEND compile_definitions_list ${compile_definitions})
      endif()

      if(overlord_LINK AND NOT "${link_options}" STREQUAL "link_options-NOTFOUND")
  ...
  if(overlord_COMPILE)
    set(${overlord_COMPILE} ${compile_options_list} PARENT_SCOPE)
  endif()

  if(overlord_LINK_OPTIONS)
    set(${overlord_LINK_OPTIONS} ${link_options_list} PARENT_SCOPE)
  endif()

  if(overlord_DEFINES)
    set(${overlord_DEFINES} ${compile_definitions_list} PARENT_SCOPE)
  endif()
...
function(toCleanedSemVer version_string)
  string(REGEX MATCH "^[0-9]+\.[0-9]+\.[0-9]+" overlord_cleaned_version "${version_string}")
  set(toCleanedSemVer_OUTPUT "${overlord_cleaned_version}" PARENT_SCOPE)
endfunction()

function(getCleanedOverlordVersion version_var)
  toCleanedSemVer("${OVERLORD_VERSION_INTERNAL}")
  set("${version_var}" "${toCleanedSemVer_OUTPUT}" PARENT_SCOPE)
endfunction()

# Get the overlord version components each in its own user defined variable
...
function(getCleanedOverlordVersionComponents major minor patch)
  getCleanedOverlordVersion("overlord_version")
  string(REPLACE "." ";" overlord_version_components "${overlord_version}")

  getVersionComponents("${overlord_version_components}" "${major}" "${minor}" "${patch}")
...
function(findClangFormat)
  ...
  if(OVERLORD_TOOLCHAIN_SYSROOT)
    if(NOT EXISTS "${OVERLORD_TOOLCHAIN_SYSROOT}/usr/bin/clang-format")
        set(error_message "Could not find clang-format in the custom toolchain sysroot, please try to install the toolchain again.")
        if(OVERLORD_ENABLE_FORMAT_ONLY)
          message(FATAL_ERROR "${error_message}")
        else()
          message(WARNING "${error_message}")
        endif()
    else()
      set(OVERLORD_CLANG_FORMAT "${OVERLORD_TOOLCHAIN_SYSROOT}/usr/bin/clang-format" CACHE FILEPATH "${clang_format_doc}")
    endif()
  else()
    find_program(OVERLORD_CLANG_FORMAT clang-format DOC "${clang_format_doc}")
    ...
    set(error_message "Could not find clang-format in the system, please install it and be sure that it can be found via the PATH env var. "
      "Otherwise provide its location by passing -DOVERLORD_CLANG_FORMAT=<clang-format-path>"
    )

    if("${OVERLORD_CLANG_FORMAT}" STREQUAL "OVERLORD_CLANG_FORMAT-NOTFOUND")
      if(OVERLORD_ENABLE_FORMAT_ONLY)
        message(FATAL_ERROR "${error_message}")
      else()
        message(WARNING "${error_message}")
  ...
  if(NOT "${OVERLORD_CLANG_FORMAT}" STREQUAL "OVERLORD_CLANG_FORMAT-NOTFOUND")
    message(STATUS "Found clang-format: ${OVERLORD_CLANG_FORMAT}")
  endif()
endfunction()
