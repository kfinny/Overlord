...
# The function creates the overlord_<c|cxx>_settings targets with compiler and linker flags
...
# if they are used only on our own targets (the ones with overlord_ prefix),
...
function(setupBuildFlags)
  ...
  if(DEFINED PLATFORM_POSIX)
    ...
    set(overlord_posix_common_defines
      POSIX=1
      OVERLORD_POSIX=1
    )
    ...
    if(OVERLORD_ENABLE_ADDRESS_SANITIZER)
    ...
    if(OVERLORD_ENABLE_THREAD_SANITIZER)
      list(APPEND posix_common_compile_options
        -fsanitize=thread
      )
    ...
    if(OVERLORD_ENABLE_LEAK_SANITIZER)
      list(APPEND posix_common_compile_options
        -fsanitize=leak
      )
      list(APPEND posix_common_link_options
    ...
    if(OVERLORD_ENABLE_ADDRESS_SANITIZER OR OVERLORD_ENABLE_THREAD_SANITIZER OR OVERLORD_ENABLE_LEAK_SANITIZER)
    ...
    list(APPEND overlord_defines
      ${overlord_posix_common_defines}
      ${posix_common_defines}
    )

    if(DEFINED PLATFORM_LINUX)
      set(overlord_linux_common_defines
        LINUX=1
        OVERLORD_LINUX=1
        OVERLORD_BUILD_DISTRO="centos7"
        OVERLORD_BUILD_PLATFORM="linux"
      )

      set(overlord_linux_common_link_options
        -Wl,-z,relro,-z,now
        -Wl,--build-id=sha1
      )
      ...
      if(OVERLORD_BUILD_FUZZERS)
        list(APPEND linux_common_compile_options
          -fsanitize=fuzzer-no-link
          -fsanitize-coverage=edge,indirect-calls
        )

        list(APPEND overlord_linux_common_defines
          OVERLORD_IS_FUZZING
        )
      endif()

      list(APPEND overlord_defines
        ${overlord_linux_common_defines}
      )
      ...
      target_link_options(cxx_settings INTERFACE
        ${overlord_linux_common_link_options}
    ...
    elseif(DEFINED PLATFORM_MACOS)
      ...
      set(overlord_macos_common_defines
        APPLE=1
        DARWIN=1
        BSD=1
        OVERLORD_DARWIN=1
        OVERLORD_BUILD_PLATFORM="darwin"
        OVERLORD_BUILD_DISTRO="10.14"
      )
      ...
      list(APPEND overlord_defines ${overlord_macos_common_defines})
    else()
      message(FATAL_ERROR "Platform not supported!")
    endif()

    if(OVERLORD_NO_DEBUG_SYMBOLS AND
  ...
  elseif(DEFINED PLATFORM_WINDOWS)
    ...
    set(overlord_windows_compile_options
      /W3
    ...
    if(OVERLORD_ENABLE_INCREMENTAL_LINKING)
    ...
    set(overlord_windows_common_defines
      WIN32=1
      WINDOWS=1
      WIN32_LEAN_AND_MEAN
      OVERLORD_WINDOWS=1
      OVERLORD_BUILD_PLATFORM="windows"
      OVERLORD_BUILD_DISTRO="10"
    ...
    if(OVERLORD_BUILD_FUZZERS)
      ...
      list(APPEND overlord_windows_common_defines
        OVERLORD_IS_FUZZING
      )
    endif()
    ...
    list(APPEND overlord_defines ${overlord_windows_common_defines})
    list(APPEND overlord_compile_options ${overlord_windows_compile_options})
  ...
  add_library(overlord_cxx_settings INTERFACE)
  target_link_libraries(overlord_cxx_settings INTERFACE
    cxx_settings
  )

  target_compile_options(overlord_cxx_settings INTERFACE
    ${overlord_compile_options}
  )

  target_compile_definitions(overlord_cxx_settings INTERFACE
    ${overlord_defines}
  )


  add_library(overlord_c_settings INTERFACE)
  target_link_libraries(overlord_c_settings INTERFACE
    c_settings
  )

  target_compile_options(overlord_c_settings INTERFACE
    ${overlord_compile_options}
  )

  target_compile_definitions(overlord_c_settings INTERFACE
    ${overlord_defines}
  )
